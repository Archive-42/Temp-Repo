...
var isSelfCrossing = function (x) {
  ...
  for (var i = 0, len = x.length; i < len; i++) {
    ...
    if (rem === 0) current.y += item;
    else if (rem === 1) current.x -= item;
    else if (rem === 2) current.y -= item;
    else current.x += item;
...
function f(a, b) {
  ...
  if (n1 === 0 && online(p1, p3, p4)) return 1;

  if (n2 === 0 && online(p2, p3, p4)) return 1;

  if (n3 === 0 && online(p3, p1, p2)) return 1;

  if (n4 === 0 && online(p4, p1, p2)) return 1;
