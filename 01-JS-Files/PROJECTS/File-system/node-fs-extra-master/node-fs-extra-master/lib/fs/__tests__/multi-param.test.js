...
describe("fs.read()", () => {
  ...
  afterEach(() => {
    return fs.close(TEST_FD).then(() => fs.remove(TEST_FILE));
  });

  describe("with promises", () => {
    it("returns an object", () => {
      return fs
        .read(TEST_FD, Buffer.alloc(SIZE), 0, SIZE, 0)
        .then((results) => {
    ...
    it("returns an object when position is not set", () => {
      return fs.read(TEST_FD, Buffer.alloc(SIZE), 0, SIZE).then((results) => {
        const bytesRead = results.bytesRead;
        const buffer = results.buffer;
        assert.strictEqual(bytesRead, SIZE, "bytesRead is correct");
        assert(buffer.equals(TEST_DATA), "data is correct");
...
describe("fs.write()", () => {
  ...
  describe("with promises", () => {
    it("returns an object", () => {
      return fs.write(TEST_FD, TEST_DATA, 0, SIZE, 0).then((results) => {
    ...
    it("returns an object when minimal arguments are passed", () => {
      return fs.write(TEST_FD, TEST_DATA).then((results) => {
        const bytesWritten = results.bytesWritten;
        const buffer = results.buffer;
        assert.strictEqual(bytesWritten, SIZE, "bytesWritten is correct");
        assert(buffer.equals(TEST_DATA), "data is correct");
    ...
    it("returns an object when writing a string", () => {
      ...
      return fs.write(TEST_FD, message).then((results) => {
        const bytesWritten = results.bytesWritten;
        const buffer = results.buffer;
        assert.strictEqual(
          bytesWritten,
...
describeNode12("fs.writev()", () => {
  ...
  describe("with promises", () => {
    ...
    it("returns an object when minimal arguments are passed", () => {
      return fs.writev(TEST_FD, TEST_DATA).then(({ bytesWritten, buffers }) => {
        assert.strictEqual(bytesWritten, SIZE, "bytesWritten is correct");
        assert.deepStrictEqual(buffers, TEST_DATA, "data is correct");
      });
    });
