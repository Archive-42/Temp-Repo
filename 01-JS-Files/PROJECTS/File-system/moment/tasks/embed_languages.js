module.exports = function (grunt) {
  grunt.registerTask("embed_languages", function () {
    ...
    var modifiedContents = momentContents.replace(
      "/* EMBED_LANGUAGES */",
      function () {
  ...
  var reTransform =
    /function \(factory\) \{[^]*\}(?=\(function \(moment\) \{)/gm;
  var replaceWith = "function (factory) {\n" + "    factory(moment);\n" + "}";
  ...
  function transformFile(file) {
    ...
    if (!fileContents.match(reTransform)) {
      grunt.warn(
        "Warning: all language files must use the common UMD wrapper pattern.  Failed language file: " +
          file
      );
      return "";
    }
