...
exports.create = {
  ...
  object: function (test) {
    ...
    test.ok(
      moment({ year: 2010, month: 1 }).toDate() instanceof Date,
      "{year: 2010, month: 1}"
    );
    test.ok(
      moment({ year: 2010, month: 1, day: 12 }).toDate() instanceof Date,
      "{year: 2010, month: 1, day: 12}"
    );
    test.ok(
      moment({ year: 2010, month: 1, day: 12, hours: 1 }).toDate() instanceof
        Date,
    ...
    test.ok(
      ...
      "{year: 2010, month: 1, hours: 12, minutes: 1, seconds: 1}"
    );
    test.ok(
      moment({
        year: 2010,
      ...
      "{year: 2010, month: 1, day: 12, hours: 1, minutes: 1, seconds: 1}"
    );
    test.ok(
      moment({
        year: 2010,
      ...
      "{year: 2010, month: 1, day: 12, hours: 1, minutes: 1, seconds: 1, milliseconds: 1}"
    );
    test.equal(
      +moment(new Date(2010, 1, 14, 15, 25, 50, 125)),
      +moment({
    ...
    test.equal(
      +moment(new Date(2010, 1, 14, 15, 25, 50, 125)),
      +moment({ y: 2010, M: 1, d: 14, h: 15, m: 25, s: 50, ms: 125 }),
  ...
  "cloning moment should only copy own properties": function (test) {
    test.expect(2);
    test.ok(
      !moment().clone().hasOwnProperty("month"),
      "Should not clone prototype methods"
    );
    test.ok(
      !moment().clone().hasOwnProperty("_lang"),
      "Should not clone prototype objects"
    );
    test.done();
  },
  ...
  "string without format": function (test) {
    ...
    test.ok(
      moment("Mon, 25 Dec 1995 13:30:00 GMT").toDate() instanceof Date,
  ...
  "first century": function (test) {
    ...
    test.equal(
      moment("99 1 1", "YYYY MM DD").format("YYYY-MM-DD"),
      "0099-01-01",
      "Year AD 99"
    );
    test.equal(
  ...
  "parsing into a language": function (test) {
    ...
    test.equal(
      moment("2012 seven", "YYYY MMM", "parselang").month(),
      6,
      "should be able to parse in a specific language"
    );
