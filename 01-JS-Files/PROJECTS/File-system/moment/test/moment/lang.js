...
exports.lang = {
  "library getter": function (test) {
    ...
    test.equal(
      moment.lang(),
      "en-gb",
      "Lang should return the changed language"
    );
  ...
  "library getter array of langs": function (test) {
    test.equal(
      moment.lang(["non-existent", "fr", "also-non-existent"]),
      "fr",
      "passing an array uses the first valid language"
    );
    test.equal(
      moment.lang(["es", "fr", "also-non-existent"]),
      "es",
      "passing an array uses the first valid language"
    );
    test.done();
  ...
  "library getter language substrings": function (test) {
    ...
    test.equal(
      moment.lang("fr-CA-does-not-exist"),
      "fr-ca",
  ...
  "library getter language array and substrings": function (test) {
    test.equal(
      moment.lang(["en-CH", "fr"]),
      "en",
      "prefer root languages to shallower ones"
    );
    test.equal(
      moment.lang(["en-gb-leeds", "en-CA"]),
      "en-gb",
      "prefer root languages to shallower ones"
    );
    test.equal(
      moment.lang(["en-fake", "en-CA"]),
      "en-ca",
      "prefer alternatives with shared roots"
    );
    test.equal(
      moment.lang(["en-fake", "en-fake2", "en-ca"]),
      "en-ca",
      "prefer alternatives with shared roots"
    );
    test.equals(
      moment.lang(["fake-CA", "fake-MX", "fr"]),
      "fr",
    ...
    test.equals(
      moment.lang(["fake-CA", "fake-MX", "fr-fake-fake-fake"]),
      "fr",
      "always find something if possible"
    );
    test.equals(
      moment.lang(["en", "en-CA"]),
      "en",
  ...
  "library ensure inheritance": function (test) {
    ...
    moment.lang("made-up", {
      ...
      months:
        "February_March_April_May_June_July_August_September_October_November_December_January".split(
          "_"
        ),
  ...
  "library ensure inheritance LT L LL LLL LLLL": function (test) {
    ...
    test.equal(
      moment().lang(lang).calendar(),
      "sameDay -LT-",
      "Should use instance lang in LT formatting"
    );
    test.equal(
      moment().add("days", 1).lang(lang).calendar(),
      "nextDay -L-",
      "Should use instance lang in L formatting"
    );
    test.equal(
      moment().add("days", -1).lang(lang).calendar(),
      "lastDay -LLL-",
      "Should use instance lang in LL formatting"
    );
    test.equal(
      moment().add("days", 4).lang(lang).calendar(),
      "nextWeek -LL-",
      "Should use instance lang in LLL formatting"
    );
    test.equal(
      moment().add("days", -4).lang(lang).calendar(),
      "lastWeek -LLLL-",
      "Should use instance lang in LLLL formatting"
    );
  ...
  "library langData": function (test) {
    ...
    test.equal(
      moment.langData().months(jan),
      "January",
      "no arguments returns global"
    );
    test.equal(
      moment.langData("zh-cn").months(jan),
      "一月",
      "a string returns the language based on key"
    );
    test.equal(
      moment.langData(moment().lang("es")).months(jan),
      "enero",
      "if you pass in a moment it uses the moment's language"
    );
  ...
  "instance lang method": function (test) {
    ...
    test.equal(
      moment([2012, 5, 6]).lang("es").format("MMMM"),
      "junio",
      "Use the instance specific language"
    );
    test.equal(
      moment([2012, 5, 6]).format("MMMM"),
      "June",
      "Using an instance specific language does not affect other moments"
    );
  ...
  "instance lang persists with manipulation": function (test) {
    ...
    test.equal(
      moment([2012, 5, 6]).lang("es").add({ days: 1 }).format("MMMM"),
      "junio",
      "With addition"
    );
    test.equal(
      moment([2012, 5, 6]).lang("es").day(0).format("MMMM"),
      "junio",
      "With day getter"
    );
    test.equal(
      moment([2012, 5, 6]).lang("es").endOf("day").format("MMMM"),
      "junio",
      "With endOf"
    );
  ...
  "duration lang method": function (test) {
    ...
    test.equal(
      moment.duration({ seconds: 44 }).lang("es").humanize(),
      "unos segundos",
      "Use the instance specific language"
    );
    test.equal(
      moment.duration({ seconds: 44 }).humanize(),
      "a few seconds",
      "Using an instance specific language does not affect other durations"
    );
  ...
  "instance lang used with from": function (test) {
    ...
    test.equal(
      b.from(a),
      "in a day",
      "do not preserve language of second moment"
    );
  ...
  "changing parts of a language config": function (test) {
    ...
    test.equal(
      ...
      "a",
      "should be able to set language values when creating the language"
    );
    ...
    test.equal(
      ...
      "a A",
      "should be able to set language values after creating the language"
    );
  ...
  "start/endOf week feature for first-day-is-monday langs": function (test) {
    ...
    test.equal(
      ...
      1,
      "for lang monday-lang first day of the week should be monday"
    );
    test.equal(
      moment([2013, 0, 1]).endOf("week").day(),
      0,
      "for lang monday-lang last day of the week should be sunday"
    );
