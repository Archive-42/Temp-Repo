...
exports.format = {
  ...
  "format timezone": function (test) {
    ...
    var b = moment(new Date(2010, 1, 14, 15, 25, 50, 125)),
      explanation =
        'moment().format("z") = ' +
        b.format("z") +
        ' It should be something like "PST"';
    if (moment().zone() === -60) {
      explanation +=
        "For UTC+1 this is a known issue, see https://github.com/timrwood/moment/issues/162";
    }
    test.ok(
  ...
  "unix timestamp": function (test) {
    ...
    test.equals(
      m.format("X"),
      "1234567890",
      "unix timestamp without milliseconds"
    );
    test.equals(
      m.format("X.S"),
      "1234567890.1",
      "unix timestamp with deciseconds"
    );
    test.equals(
      m.format("X.SS"),
      "1234567890.12",
      "unix timestamp with centiseconds"
    );
    test.equals(
      m.format("X.SSS"),
      "1234567890.123",
  ...
  zone: function (test) {
    ...
    if (moment().zone() > 0) {
      test.ok(
        moment().format("ZZ").indexOf("-") > -1,
        "When the zone() offset is greater than 0, the ISO offset should be less than zero"
      );
    }
    if (moment().zone() < 0) {
      test.ok(
        moment().format("ZZ").indexOf("+") > -1,
        "When the zone() offset is less than 0, the ISO offset should be greater than zero"
      );
    }
    if (moment().zone() === 0) {
      test.ok(
        moment().format("ZZ").indexOf("+") > -1,
        "When the zone() offset is equal to 0, the ISO offset should be positive zero"
      );
    }
    if (moment().zone() === 0) {
      test.equal(
        moment().zone(),
        0,
        "moment.fn.zone should be a multiple of 15 (was " +
          moment().zone() +
          ")"
      );
    } else {
    ...
    test.equal(
      ...
      "zone should equal getTimezoneOffset"
    );
    test.done();
  },
  ...
  "escaping quotes": function (test) {
    ...
    test.equal(
      date.format("MMM 'YY"),
      "Jan '12",
      "Should be able to format with single parenthesis"
    );
    test.equal(
      date.format('MMM "YY'),
      'Jan "12',
      "Should be able to format with double parenthesis"
    );
    test.equal(
      date.format("MMM 'YY"),
      "Jan '12",
  ...
  toJSON: function (test) {
    var supportsJson =
        typeof JSON !== "undefined" && JSON.stringify && JSON.stringify.call,
    ...
    test.equal(
      ...
      "should output ISO8601 on moment.fn.toJSON"
    );
  ...
  toISOString: function (test) {
    ...
    test.equal(
      ...
      "ISO8601 format on negative year"
    );
    // big negative years
    date = moment.utc("-020123-10-09T20:30:40.678");
    test.equal(
