...
exports["lang:tr"] = {
  ...
  format: function (test) {
    var a = [
        [
          ...
          "Pazar, Şubat 14'üncü 2010, 3:25:50 pm",
        ],
        ["ddd, hA", "Paz, 3PM"],
    ...
    for (i = 0; i < DDDo.length; i++) {
      DDDoDt = moment([2010]);
      test.equal(
        DDDoDt.add("days", DDDo[i][0]).format("DDDo"),
        DDDo[i][1],
        DDDo[i][0] + " ---> " + DDDo[i][1]
      );
    }
  ...
  "format week": function (test) {
    ...
    var expected =
        "Pazar Paz Pz_Pazartesi Pts Pt_Salı Sal Sa_Çarşamba Çar Ça_Perşembe Per Pe_Cuma Cum Cu_Cumartesi Cts Ct".split(
          "_"
        ),
      i;
    for (i = 0; i < expected.length; i++) {
  ...
  "calendar last week": function (test) {
    ...
    for (i = 2; i < 7; i++) {
      ...
      test.equal(
        m.calendar(),
        m.format("[geçen hafta] dddd [saat] LT"),
        "Today - " + i + " days current time"
      );
      m.hours(0).minutes(0).seconds(0).milliseconds(0);
      test.equal(
