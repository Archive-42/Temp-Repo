...
exports["lang:it"] = {
  ...
  format: function (test) {
    ...
    var a = [
        [
          ...
          "Domenica, Febbraio 14º 2010, 3:25:50 pm",
        ],
        ["ddd, hA", "Dom, 3PM"],
  ...
  "format week": function (test) {
    ...
    var expected =
        "Domenica Dom D_Lunedì Lun L_Martedì Mar Ma_Mercoledì Mer Me_Giovedì Gio G_Venerdì Ven V_Sabato Sab S".split(
          "_"
        ),
      i;
    for (i = 0; i < expected.length; i++) {
  ...
  "calendar last week": function (test) {
    ...
    for (i = 2; i < 7; i++) {
      ...
      test.equal(
        m.calendar(),
        m.format("[lo scorso] dddd [alle] LT"),
        "Today - " + i + " days current time"
      );
      m.hours(0).minutes(0).seconds(0).milliseconds(0);
      test.equal(
