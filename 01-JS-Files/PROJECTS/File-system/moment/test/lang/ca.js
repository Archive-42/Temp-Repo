...
exports["lang:ca"] = {
  ...
  parse: function (test) {
    ...
    var tests =
        "gener gen._febrer febr._març mar._abril abr._maig mai._juny jun._juliol jul._agost ag._setembre set._octubre oct._novembre nov._desembre des.".split(
          "_"
        ),
      i;
  ...
  "calendar last week": function (test) {
    ...
    for (i = 2; i < 7; i++) {
      ...
      test.equal(
        m.calendar(),
        m.format(
          "[el] dddd [passat a " + (m.hours() !== 1 ? "les" : "la") + "] LT"
        ),
        "Today - " + i + " days current time"
      );
      m.hours(0).minutes(0).seconds(0).milliseconds(0);
