...
exports["lang:nn"] = {
  ...
  parse: function (test) {
    ...
    var tests =
        "januar jan_februar feb_mars mar_april apr_mai mai_juni jun_juli jul_august aug_september sep_oktober okt_november nov_desember des".split(
          "_"
        ),
      i;
  ...
  "format week": function (test) {
    ...
    var expected =
        "sundag sun su_måndag mån må_tysdag tys ty_onsdag ons on_torsdag tor to_fredag fre fr_laurdag lau lø".split(
          "_"
        ),
      i;
    for (i = 0; i < expected.length; i++) {
  ...
  "calendar last week": function (test) {
    ...
    for (i = 2; i < 7; i++) {
      ...
      test.equal(
        m.calendar(),
        m.format("[Føregående] dddd [klokka] LT"),
        "Today - " + i + " days current time"
      );
      m.hours(0).minutes(0).seconds(0).milliseconds(0);
      test.equal(
