...
exports["lang:zh-cn"] = {
  ...
  "calendar current week": function (test) {
    ...
    for (i = 0; i < 7; i++) {
      ...
      test.equal(
        m.calendar(),
        m.format("[本]ddd凌晨12点整"),
        "Monday + " + i + " days current time"
      );
    }
    test.done();
  },
  ...
  "calendar last week": function (test) {
    ...
    for (i = 1; i < 8; i++) {
      ...
      if (
        Math.abs(m.diff(today, "days")) >= 7 ||
        Math.abs(m.diff(today, "days")) <= 1
      ) {
        continue;
      }
      test.equal(
        m.calendar(),
        m.format("[上]ddd凌晨12点整"),
        "Monday - " + i + " days next week"
      );
    }
    test.done();
  },
